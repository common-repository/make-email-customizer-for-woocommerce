...
class EC_Shortcodes {
	...
	public static function ec_order( $shortcode_args ) {
		...
		if ( ! isset( $ec_email_args['order'] ) ) {
			if ( isset( $_REQUEST["ec_render_email"] ) ) return '<span class="shortcode-error">' . __( 'This shortcode cannot be used in this email', 'make' ) . '</span>';
			else return;
		}
		
		// Check if necessary email args exits
		if ( ! isset( $ec_email_args['order'] ) ) {
			if ( isset( $_REQUEST["ec_render_email"] ) ) return '<span class="shortcode-error">[' . __( 'Order shortcodes cannot be used in this email', 'make' ) . ']</span>';
			else return;
		}
	...
	public static function ec_reset_password_link( $shortcode_args ) {
		...
		$text = __( 'Click here to reset your password', 'make' );
		if ( isset( $shortcode_args_modified['text'] ) && trim( $shortcode_args_modified['text'] ) )
			$text = __( $shortcode_args_modified['text'], 'make' );
