...
class EC_Settings {
			...
			switch( $value['type'] ) {
						...
						<div class="controls-field">
							<div class="controls-inner-row">
								...
								<?php if ( $value['default'] ) : ?>
									<span class="reset-to-default" title="<?php echo esc_attr( __( "Reset to:", 'make' ) . "\n" . $value['default'] ); ?>" data-default="<?php echo esc_attr( $value['default'] ); ?>">
										<?php _e( 'Reset to default', 'make' ) ?> <i class="cxectrl-icon-arrows-cw"></i>
									</span>
								<?php endif ?>
				...
				case 'image_width' :
					...
					?><tr valign="top">
						...
						<td class="forminp image_width_settings">
							...
							<label><input name="<?php echo esc_attr( $value['id'] ); ?>[crop]" id="<?php echo esc_attr( $value['id'] ); ?>-crop" type="checkbox" <?php echo $crop; ?> /> <?php _e( 'Hard Crop?', 'make' ); ?></label>
				...
				case 'single_select_page' :
					...
					?><tr valign="top" class="single_select_page">
						<th scope="row" class="titledesc"><?php echo esc_html( $value['title'] ) ?> <?php echo $tip; ?></th>
						<td class="forminp">
							<?php echo str_replace(' id=', " data-placeholder='" . __( 'Select a page&hellip;', 'make' ) .  "' style='" . $value['css'] . "' class='" . $value['field_class'] . "' id=", wp_dropdown_pages( $args ) ); ?> <?php echo $description; ?>
						</td>
					</tr><?php
				break;
				...
				case 'single_select_country' :
					...
					?><tr valign="top">
						...
						<td class="forminp"><select name="<?php echo esc_attr( $value['id'] ); ?>" style="<?php echo esc_attr( $value['css'] ); ?>" data-placeholder="<?php _e( 'Choose a country&hellip;', 'make' ); ?>" title="Country" class="chosen_select">
							<?php
							if ( class_exists('WC') )
				...
				case 'multi_select_countries' :
					...
					?><tr valign="top">
						...
						<td class="forminp">
							<select multiple="multiple" name="<?php echo esc_attr( $value['id'] ); ?>[]" style="width:350px" data-placeholder="<?php _e( 'Choose countries&hellip;', 'make' ); ?>" title="Country" class="chosen_select">
								<?php
									if ( $countries )
										foreach ( $countries as $key => $val )
											echo '<option value="' . esc_attr( $key ) . '" ' . selected( in_array( $key, $selections ), true, false ).'>' . $val . '</option>';
								?>
							</select> <?php if ( $description ) echo $description; ?> </br><a class="select_all button" href="#"><?php _e( 'Select all', 'make' ); ?></a> <a class="select_none button" href="#"><?php _e( 'Select none', 'make' ); ?></a>
						</td>
					</tr><?php
				break;
	...
	public static function ec_render_option( $option_name, $option_value = '' ) {
		...
		if ( in_array( self::get_option_array( $option_name, 'type' ), array( 'text', 'textarea' ) ) ) {
			...
			$option_value = __( $option_value, 'make' );
		}
