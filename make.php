<?php
/*
Plugin Name: Make Email Customizer for WooCommerce
 ...
 * Define Constants
 */
define( 'WC_EMAIL_CONTROL_VERSION', '1.0.4' );
...
define( 'WC_EMAIL_CONTROL_PLUGIN_BASENAME', plugin_basename( __FILE__ ) ); // make-email-customizer-for-woocommerce/make.php
...
include_once( 'ec-theme-yellow.php' ); // Theme
...
class WC_Email_Control {
	...
	*/
	function save_meta() {
		...
		*/
		...
		$field_value  = ( isset( $_REQUEST["field_value"] ) ) ? sanitize_text_field( $_REQUEST["field_value"] ) : "" ;
	...
	*/
	function ec_save_option() {
		...
		$field_name = ( isset(  $_REQUEST['field_name'] ) ) ?  sanitize_text_field( $_REQUEST['field_name'] ) : '' ;
		$field_value = ( isset(  $_REQUEST['field_value'] ) ) ?  sanitize_text_field( $_REQUEST['field_value'] ) : '' ;
		$field_type = ( isset(  $_REQUEST['field_type'] ) ) ?  sanitize_text_field( $_REQUEST['field_type'] ) : '' ; // option | user
	...
	*/
	public function send_email() {
		...
		$email_type = sanitize_email( $_REQUEST['ec_email_type'] );
		$order_id   = sanitize_email( $_REQUEST['ec_email_order'] );
			...
			if ( ! empty( $mails ) ) {
				foreach ( $mails as $mail ) {
					if ( $mail->id == $email_to_send ) {
						...
						$mail->recipient = ( isset( $_REQUEST['ec_email_addresses'] ) ? sanitize_email($_REQUEST['ec_email_addresses'] ) : NULL );
	...
	*/
	public function save_edit_email() {
		...
		$email_type		= ( $_REQUEST['ec_email_type'] ) ? sanitize_text_field( $_REQUEST['ec_email_type'] ) : false ;
		$email_id		= ( $_REQUEST['ec_email_id'] ) ? sanitize_text_field( $_REQUEST['ec_email_id'] ) : false ;
	...
	function woocommerce_settings_button( $data ) {
		...
		<div class="pe-wc-settings-holder">
			...
			<?php if ( isset( $_REQUEST["section"] ) && $_REQUEST["section"] != "" ) { ?>
				...
				<p>
					<?php _e( "Preview and test emails as they will appear in mail clients when received.", 'make' ) ?> &nbsp; &nbsp;
		...
		if ( ! empty( $cxec_plugins_found ) ) :
			...
			<div id="message" class="error">
				<p>
					<?php
					printf(
						__( '%sEmail Customizer for WooCommerce is inactive due to conflicts%sOur plugin will conflict with the following plugins and cannot be used while they are active: %s', 'make' ),
						'<strong>',
						'</strong><br>',
			...
			if ( ! class_exists( 'WooCommerce' ) ) :
				...
				<div id="message" class="error">
					<p>
						<?php
						printf(
							__( '%sEmail Customizer for WooCommerce needs WooCommerce%s %sWooCommerce%s must be active for Email Customizer to work. Please install & activate WooCommerce.', 'make' ),
							'<strong>',
							'</strong><br>',
			...
			elseif ( version_compare( get_option( 'woocommerce_db_version' ), WC_EMAIL_CONTROL_REQUIRED_WOOCOMMERCE_VERSION, '<' ) ) :
				...
				<div id="message" class="error">
					<p>
						<?php
						printf(
							__( '%sEmail Customizer for WooCommerce is inactive%s This version of Email Customizer requires WooCommerce %s or newer. For more information about our WooCommerce version support %sclick here%s.', 'make' ),
							'<strong>',
							'</strong><br>',
