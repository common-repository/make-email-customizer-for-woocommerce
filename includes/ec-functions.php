...
if ( ! function_exists( 'ec_register_email_theme' ) ) {
	function ec_register_email_theme( $theme_id, $args ) {
		...
		if ( strlen( $theme_id ) > 40 ) {
			_doing_it_wrong( __FUNCTION__, __( 'Theme IDs cannot exceed 20 characters in length', 'make' ) );
			return new WP_Error( 'theme_id_too_long', __( 'Theme IDs cannot exceed 20 characters in length', 'make' ) );
		}
