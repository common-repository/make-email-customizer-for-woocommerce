...
<div class="wp-core-ui ec ec-admin-page pe-in-admin-page">
	...
	<div class="left-column">
		...
		<div class="ec-admin-panel ec-admin-panel-controls">
			...
			<form class="main-controls-form" id="render_email" name="render_email" data-name="Render Email"  action="<?php echo wp_nonce_url( admin_url( $src_url ), 'make'); ?>" target="preview-email-iframe" method="post" >
				<div class="main-controls">
					
					<div class="main-controls-element main-controls-element-heading-block">
						<div class="heading-first">
							<h2><?php _e( "Email Customizer", 'make' ); ?></h2>
						</div>
					</div>
					...
					if ( isset( $ec_email_themes ) && is_array( $ec_email_themes ) ) :
						...
						<div class="main-controls-element">
							<label class="controls-label" for="ec_email_theme" title="<?php _e( "Choose which email theme to preview. Then click 'Use' to use it for all your future WooCommerce emails.", 'make' ); ?>">
								<?php _e( "Email Theme to show", 'make' ); ?> 
							...
							<div class="controls-field">
								<div class="controls-inner-row">
									<select class="w-select control-dropdown" id="ec_email_theme" name="ec_email_theme">
										<option value="woocommerce_original">
											<?php _e( 'WooCommerce (original, not editable)', 'make' ); ?>
										</option>
										...
										foreach ( $ec_email_themes as $ec_email_theme_id => $ec_email_theme_args ) {
											...
											<option class="theme-select" value="<?php echo $ec_email_theme_id; ?>" <?php echo ( $ec_email_theme_id == $ec_theme_selected ) ? "selected" : "" ; ?> >
												<?php echo $name; ?><?php if ( ! $woocommerce_version_pass ) echo ' ' . __( '(WooCommerce update required)', 'make' ) ?>
											</option>
											<?php
										}
										?>
								...
								<div class="controls-inner-row controls-inner-button-row ec_theme_button_row" id="theme-commit">
									<a class="button" id="ec_cancel_email_theme" href="#"><i class="cxectrl-icon-reply"></i>&nbsp; <?php _e( "Don't Use", 'make' ); ?></a>
									<a class="button button-primary" id="ec_save_email_theme" href="#"><i class="cxectrl-icon-check"></i>&nbsp; <?php _e( "Use", 'make' ); ?></a>
								</div>
							</div>
						</div>
						<?php
					...
					<div class="main-controls-element">
						<label class="controls-label" for="ec_email_type" title="<?php _e( 'Choose which email type to preview.', 'make' ); ?>">
							<?php _e( "Email Type to show", 'make' ); ?> 
							<span class="help-icon">&nbsp;&nbsp;&nbsp;&nbsp;</span>
						</label>
						<div class="controls-field">
							<div class="controls-inner-row">
					...
					<div class="main-controls-element" id="ec_edit_content_controls">
						<label class="controls-label" for="ec_edit_content" title="<?php _e( "Customize the email you're showing in the preview. Then click 'Save & Publish' to save your customizations.", 'make' ); ?>">
							<?php _e( "Customize", 'make' ); ?> 
							<span class="help-icon">&nbsp;&nbsp;&nbsp;&nbsp;</span>
						</label>
						<div class="controls-field">
							<div class="controls-inner-row">
								<button class="button" id="ec_edit_content" href="#">
									<i class="cxectrl-icon-pencil"></i> <?php _e( "Customize", 'make' ); ?>
								</button>
							</div>
						</div>
						<?php if ( false ) : ?>
							<div class="get_themes button"><?php _e( "Get Customized Themes", 'make' ); ?></div>
					...
					<div class="main-controls-element">
						<label class="controls-label" for="ec_email_order" title="<?php _e( 'Choose which order to use to populate the email preview.', 'make' ); ?>">
							<?php _e( "Order to show", 'make' ); ?> 
							<span class="help-icon">&nbsp;&nbsp;&nbsp;&nbsp;</span>
						</label>
						<div class="controls-field">
							<div class="controls-inner-row">
								...
								<select class="w-select control-dropdown" id="ec_email_order" name="ec_email_order">
									...
									if ( count( $order_collection ) ) {
										...
										?>
										<option value="">
											<?php _e( "There are no orders to preview...", 'make' ); ?>
										</option>
										<?php
									}
									...
									if ( $limit_orders <= count( $order_collection ) ) {
										?>
										<option><?php printf( __( '...Showing the most recent %u orders', 'make' ), $limit_orders ); ?></option>
										<?php
									}
									?>
								</select>
					...
					<div class="main-controls-element">
						<label class="controls-label" title="<?php _e( 'More advanced debugging options.', 'make' ); ?>">
							<?php _e( "Debugging", 'make' ) ; ?> 
							<span class="help-icon">&nbsp;&nbsp;&nbsp;&nbsp;</span>
						</label>
						<div class="controls-field">
							<div class="controls-inner-row">
								...
								<label for="header_info_userspecifc">
									...
									<?php _e( 'Show Header & Template Info', 'make' ) ?>
								</label>
					...
					<div class="main-controls-element">
						...
						?>
						<label class="controls-label" for="ec_send_email" title="<?php _e( 'Send a test email to any address. Use a comma separated list to send to multiple addresses', 'make' ); ?>">
							<?php _e( "Send a Test", 'make' ) ; ?> 
							<span class="help-icon">&nbsp;&nbsp;&nbsp;&nbsp;</span>
						</label>
						<div class="controls-field">
							<div class="controls-inner-row">
					...
					if ( ! empty( $ec_email_types_for_settings) && ec_get_settings( $ec_email_theme_id ) ) {
						...
						<form
							...
							<div class="main-controls-element forminp-tags ec-allowed-tags">
								<label class="controls-label" title="<?php echo esc_attr( __( 'Copy & paste any of these [shortcodes] to use dynamic text in your text.', 'make' ) ) ?>">
									<?php _e( 'Allowed Shortcodes:', 'make' ); ?>
									<span class="help-icon">&nbsp;</span>
								</label>
								<div class="controls-field">
									<div class="controls-inner-row">
										...
										[ec_shipping_method] 
										[ec_payment_method] 
										<!-- [ec_custom_field] --> <!-- <span class="ec-new-shortcode-badge"><?php _e( 'New', 'make' ) ?></span>  -->
										
										<!--p class="ec-allowed-shortcode-docs-link"><?php echo sprintf( __( 'For more shortcode documentation <a href="%s" target="_blank">click here</a>', 'make' ), '#' ); ?></p-->
									</div>
								</div>
							</div>
	...
	<!-- Main Content -->
	<iframe id="preview-email-iframe" name="preview-email-iframe" border="0" src="<?php echo wp_nonce_url( admin_url( $src_url ), 'make'); ?>"></iframe>

	<a class="ec-propout-preview button" href="#" target="_blank" title="<?php _e( 'Popout Email Preview', 'make' ) ?>">
